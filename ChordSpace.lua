...
function ChordSpace.barycentricCoordinates(chord, simplex)
    ...
    local simplexVolume = ChordSpace.volumeSquared(simplex)
    for voice = 1, #coordinates do
        ...
        local voiceVolume = ChordSpace.volumeSquared(voiceSimplex)
...
-- Returns whether the point lies in the simplex or on its boundary.
-- The point and the vertices of the simplex must have the same dimension,
-- but the simplex may define a subspace.

function ChordSpace.inSimplex(chord, simplex)
end

-- Returns a simplex defining the cyclical hyperplane at the base of the
-- orbifold, whose convex hull can be generated by translating the base by
-- range / N.

backwards = true

function Chord:cyclicalRegion(range)
